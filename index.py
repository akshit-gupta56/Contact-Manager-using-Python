...
def UpdateData():
    if GENDER.get() == "":
        ...
        conn = sqlite3.connect("contactdb.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE `member` SET `firstname` = ?, `lastname` = ?, `gender` = ?, `age` = ?, `address` = ?, `contact` = ? WHERE `mem_id` = ?", (str(FIRSTNAME.get()), str(LASTNAME.get()), str(AGE.get()), str(GENDER.get()), str(ADDRESS.get()), str(CONTACT.get()), int(mem_id)))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def DeleteData():
    if not tree.selection():
        ...
        result = tkMessageBox.askquestion('Confirm Deletion', 'Are you sure you want to delete this record?', icon = 'warning')
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
...
def AddNewWindow():
    ...
    #----------LABELS----------#
    lbl_title = Label(FormTitle, text = "Adding New Contacts", font = ('arial', 16), bg = "#03dac5", width = 300)
    lbl_title.pack(fill = X)
    lbl_firstname = Label(ContactForm, text = "Firstname", font = ('arial', 14), bd = 5)
    lbl_firstname.grid(row = 0, sticky = W)
    lbl_lastname = Label(ContactForm, text = "Lastname", font = ('arial', 14), bd = 5)
...
btn_add.pack()
btn_delete = Button( MidRight, text = "DELETE EXISTING", bg = "#cf6679", command = DeleteData)
